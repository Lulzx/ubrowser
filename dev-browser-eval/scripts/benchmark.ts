...
const CLAUDE_PATH = "/Users/lulzx/.local/bin/claude";
...
const TEST_CASES: Record<string, TestCase> = {
  "game-tracker": {
    ...
    prompt: (instruction) =>
      `Start the dev server (bun run dev) and ${instruction} to create an account (with a unique and secure email and password), login, add 5 games (rate them all a 9), and view the collection. The app runs on localhost:3000 with routes: /register (email+password form), /add (name input + rating select 1-10), /collection. MINIMIZE TURNS - batch everything possible.`,
    reset: resetDev,
  },
};

type Method = "dev-browser" | "playwright-skill" | "playwright-mcp" | "ubrowser";
...
const METHODS: Record<Method, string> = {
  ...
  "ubrowser": `use ubrowser (μBrowser). CRITICAL EFFICIENCY RULES:
1. Use browser_batch to combine ALL actions into as FEW calls as possible
2. A single batch can have 20+ steps - combine register+login, then ALL 5 games in ONE batch
3. Example for adding 5 games in ONE batch:
   {"steps": [
     {"tool": "navigate", "args": {"url": "/add"}},
     {"tool": "type", "args": {"selector": "input[name='name']", "text": "Game1"}},
     {"tool": "select", "args": {"selector": "select[name='rating']", "value": "9"}},
     {"tool": "click", "args": {"selector": "button[type='submit']"}},
     {"tool": "navigate", "args": {"url": "/add"}},
     {"tool": "type", "args": {"selector": "input[name='name']", "text": "Game2"}},
     ... repeat for all 5 games ...
   ], "snapshot": {"when": "final"}}
4. Only request snapshot at the VERY END when viewing collection
5. NEVER read code files - go straight to browser actions`,
};
...
> = {
  ...
  "ubrowser": {
    plugins: {
      ...
      allowedMcpServers: [{ serverName: "ubrowser" }],
    },
    mcp: {
      mcpServers: {
        ubrowser: {
          type: "stdio",
          command: "node",
          args: ["/Users/lulzx/work/browser-skill/build/index.js"],
        },
      },
    },
  },

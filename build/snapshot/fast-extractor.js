...
async function ensureInjected(page) {
    if (injectedPages.has(page))
        return;
    try {
        // Check if already injected (page might have been reloaded)
        const hasScript = await page.evaluate(() => typeof window.__ubrowserExtract === 'function');
        if (!hasScript) {
...
export async function extractInteractiveElementsFast(page, scope, options) {
    ...
    if (!options?.skipCache) {
        ...
        if (cached) {
            ...
            const mutationCount = await page.evaluate(() => window.__ubrowserMutationCount);
    ...
    const result = await page.evaluate(([s, m]) => window.__ubrowserExtract(s, m), [scope || 'body', maxElements]);
    // Update cache
    const mutationCount = await page.evaluate(() => window.__ubrowserMutationCount);
...
//# sourceMappingURL=fast-extractor.js.map
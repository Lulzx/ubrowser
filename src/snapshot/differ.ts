...
export function computeSnapshotHash(elements: ElementRef[]): string {
...
export function computeDiff(
  ...
  const prevMap = new Map<string, ElementRef>();
  for (const el of previous.elements) {
    prevMap.set(el.id, el);
  }

  const currMap = new Map<string, ElementRef>();
  ...
  for (const el of current) {
    ...
    if (!prev) {
      ...
      if (prev.name !== el.name) changes.push(`name: "${el.name}"`);
      if (prev.role !== el.role) changes.push(`role: ${el.role}`);
...
function formatElementAsHTML(el: ElementRef): string {

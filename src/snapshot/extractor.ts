...
export async function extractInteractiveElements(page: Page, scope?: string): Promise<Array<{
  ...
  const elements = await page.evaluate((scopeSel) => {
    ...
    for (const el of els) {
      ...
      if (disabled !== null) attributes.disabled = 'true';
      if (checked !== null) attributes.checked = 'true';
      if (required !== null) attributes.required = 'true';
    ...
    function getImplicitRole(tag: string, type: string | null): string {

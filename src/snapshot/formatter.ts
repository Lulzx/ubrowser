...
//   a#e3/login"Sign in"
//   sel#e4"Country"
export function formatElementsCompact(elements: ElementRef[]): string {
...
function formatElementCompact(el: ElementRef): string {
  ...
  if (el.attributes?.disabled) result += '!d';
  if (el.attributes?.checked) result += '!c';
  if (el.attributes?.required) result += '!r';

  return result;
}

// Format elements as HTML-style string (fallback, 56% more token efficient than JSON)
export function formatElementsAsHTML(elements: ElementRef[]): string {
  return elements.map(formatElement).join('\n');
}

// Format a single element (HTML style)
function formatElement(el: ElementRef): string {
...
function isRoleImpliedByTag(tag: string, role: string): boolean {
...
// Snapshot format type - 'compact' is now default for maximum token savings
...
function formatCompactSnapshot(elements: ElementRef[], url: string, title: string): string {
  // Short header
...
function formatFullSnapshot(elements: ElementRef[], url: string, title: string): string {
...
export function getLastSnapshot(): PrunedSnapshot | null {
  return lastSnapshot;
}
